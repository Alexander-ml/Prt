          ...
          <SectionCard icon="bi-egg-fried" title="Selección de Platos para Comanda">
            <div className="d-flex flex-column gap-3 mb-3">
              ...
              <div className="d-flex gap-2 overflow-x-auto pb-1">
                <button
                  type="button"
                  aria-pressed={selectedDishCategory === 'todas'}
                  className={`btn btn-sm text-nowrap fw-semibold flex-shrink-0 ${selectedDishCategory === 'todas' ? 'btn-primary' : 'btn-outline-secondary bg-white'}`}
                ...
                {categories.map(c => (
                  <button
                    type="button"
                    key={c.id}
                    aria-pressed={selectedDishCategory === c.id}
                    className={`btn btn-sm text-nowrap fw-semibold flex-shrink-0 ${selectedDishCategory === c.id ? 'btn-primary' : 'btn-outline-secondary bg-white'}`}
                    style={{ borderRadius: 8 }}
            ...
            <div className="row g-3" style={{ maxHeight: '600px', overflowY: 'auto' }}>
              {filteredDishes.length === 0 ? (
                ...
                filteredDishes.map(dish => {
                  const inCart = cartItems.find(ci => ci.dish.id === dish.id);
                  return (
                    <div key={dish.id} className="col-12 col-sm-6 col-md-4">
                      <div
                        className={`position-relative p-3 rounded-3 border bg-white h-100 d-flex flex-column justify-content-between ${
                          !dish.isAvailableToday ? 'opacity-50' : 'cursor-pointer'
                        } ${inCart ? 'border-primary' : ''}`}
                        style={{
                          transition: 'transform 0.15s ease, box-shadow 0.15s ease',
                          boxShadow: inCart ? 'var(--shadow-sm)' : 'var(--shadow-xs)',
                          borderWidth: inCart ? 2 : 1,
                        }}
                        role={dish.isAvailableToday ? 'button' : undefined}
                        tabIndex={dish.isAvailableToday ? 0 : -1}
                        aria-label={`Agregar ${dish.name} a la comanda`}
                        onClick={() => dish.isAvailableToday && handleAddToCart(dish)}
                        onKeyDown={e => {
                          if (dish.isAvailableToday && (e.key === 'Enter' || e.key === ' ')) {
                            e.preventDefault();
                        ...
                        onMouseLeave={e => {
                          e.currentTarget.style.boxShadow = inCart ? 'var(--shadow-sm)' : 'var(--shadow-xs)';
                      ...
                      >
                        ...
                        <div>
                          ...
                          <span className="fw-bold d-block mb-2" style={{ color: 'var(--color-brand)', fontSize: '1rem' }}>
                        ...
                        <button
                          type="button"
                          className={`btn btn-sm w-100 fw-semibold ${inCart ? 'btn-primary' : 'btn-outline-primary'}`}
                        ...
                        >
                          <i className="bi bi-plus-lg me-1"></i> {inCart ? 'Añadir otro' : 'Agregar'}
                        </button>
                      </div>
                    </div>
                  );
        ...
        <div className="col-12 col-lg-5 col-xl-4">
          ...
          >
            <div className="border-bottom pb-3 mb-3">
              <label className="form-label fw-bold" htmlFor="mesaSelect">1. Seleccionar Mesa</label>
              <select
                id="mesaSelect"
              ...
              {selectedTableObj && (
                <div className="d-flex align-items-center justify-content-between mt-2">
                  <small style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                    ...
                    Capacidad: {selectedTableObj.capacity} pers.
                  </small>
                  <Badge
                    status={selectedTableObj.status.toUpperCase()}
                    variant={selectedTableObj.status === 'ocupada' ? 'warning' : 'success'}
                  />
                </div>
              )}
            </div>
